...
export default function IncidentsComponent() {
  ...
  const correlatedDeployments = new Set(
    orgBrainData.incidents.flatMap((incident) => incident.correlatedDeploymentIds ?? []),
  ).size;
    ...
    <div className="min-h-full bg-background/40">
      ...
      <div className="mx-auto w-full max-w-[1680px] space-y-6 p-5 sm:p-6">
        <div className="space-y-3">
          <SectionLabel aside="Live from deterministic fixtures">Operations pulse</SectionLabel>
          <div className="portal-grid grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <MetricCard icon={Siren} label="Incidents" value={orgBrainData.incidents.length} hint="Current dataset" />
            <MetricCard icon={ShieldAlert} label="Open investigations" value={activeCount} hint="Not resolved or closed" />
            <MetricCard icon={Activity} label="Affected services" value={affectedServices} hint="Unique services" />
            <MetricCard icon={GitCommitHorizontal} label="Correlated deployments" value={correlatedDeployments} hint="Deployment-linked incidents" />
          </div>
        </div>

        <div className="space-y-3">
          <SectionLabel aside="Select an incident to inspect evidence">Incident stream</SectionLabel>
          <div className="portal-grid grid gap-4 xl:grid-cols-2">
            {orgBrainData.incidents.map((incident) => {
              ...
              return (
                <Link key={incident.id} href={`/incidents/${incident.id}`} className="group outline-none">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="grid gap-3 rounded-xl border bg-muted/20 p-3 text-xs text-muted-foreground sm:grid-cols-3">
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          <span className="truncate">{new Date(incident.startedAt).toLocaleString()}</span>
                        </div>
                        <div className="flex min-w-0 items-center gap-2">
                          <GitCommitHorizontal className="size-3.5 shrink-0" />
                          <span className="truncate">{primaryDeployment?.version ?? "No deployment"}</span>
                        </div>
                        <div className="flex min-w-0 items-center gap-2">
                          <RadioTower className="size-3.5 shrink-0" />
                          <span className="truncate">{incident.traceIds?.length ?? 0} trace</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
