...
export default function ScenarioLabComponent() {
  ...
  const signalCount = new Set(publicScenarios.flatMap((scenario) => scenario.signals)).size;
    ...
    <div className="min-h-full bg-background/40">
      ...
      <div className="mx-auto w-full max-w-[1680px] space-y-6 p-5 sm:p-6">
        <div className="space-y-3">
          <SectionLabel aside="Safe to demo repeatedly">Fixture readiness</SectionLabel>
          <div className="portal-grid grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <MetricCard icon={Sparkles} label="Scenarios" value={publicScenarios.length} hint="Defined fixtures" />
            <MetricCard icon={CheckCircle2} label="Ready to inject" value={readyCount} hint="Complete deterministic evidence" />
            <MetricCard icon={Activity} label="Signal types" value={signalCount} hint="Logs, traces, metrics and changes" />
            <MetricCard icon={BadgeAlert} label="Active fixture" value={active ? "1" : "0"} hint={active ? "Injected in this session" : "No active scenario"} />
          </div>
        </div>

        <div className="space-y-3">
          <SectionLabel aside="The root cause remains hidden from the client">Scenario catalogue</SectionLabel>
                    ...
                    {injected && incidentId ? (
                      <div className="animate-in fade-in slide-in-from-bottom-2 rounded-xl border bg-muted/25 p-3 text-sm duration-300">
                        ...
                        <p className="mt-2 text-xs leading-5 text-muted-foreground">
                          Observable evidence is now available to the investigation surface. The evaluation answer key remains isolated.
                        </p>
                        <Button
