...
const toolMeta = {
  ...
  inspect_source_changes: {
    label: "Inspected source changes",
    icon: FileCode2,
  },
  correlate_change: { label: "Correlated change", icon: Network },
  inspect_architecture_decisions: {
    label: "Inspected architecture decisions",
    icon: BookOpenCheck,
  },
  check_architecture_constraints: {
    label: "Checked architecture constraints",
    icon: ShieldAlert,
  },
  resolve_dependency_graph: {
    label: "Resolved dependency graph",
    icon: Network,
  },
  ...
  prepare_remediation_work: {
    label: "Prepared remediation work",
    icon: Wrench,
  },
...
function outputSummary(name: string, output: unknown): string {
  ...
  if (
    name === "start_investigation_workflow" &&
  ...
  if (
    ...
    return `${value.decisions.length} decision${value.decisions.length === 1 ? "" : "s"}`;
  if (
    name === "check_architecture_constraints" &&
    Array.isArray(value.constraints)
  )
    return `${value.constraints.length} constraint${value.constraints.length === 1 ? "" : "s"}`;
  if (name === "resolve_dependency_graph" && Array.isArray(value.edges))
    return `${value.edges.length} edges`;
  if (name === "assess_blast_radius" && typeof value.count === "number")
    return `${value.count} downstream services`;
  ...
  if (name === "prepare_remediation_work" && value.execution === "draft-only")
    return "Draft only";
  if (name === "record_approval" && typeof value.status === "string")
    return value.status.replaceAll("-", " ");
...
export function OrgToolRenderer({
  name,
  output,
  status,
