...
export const mockProviders: OrgBrainProviders = {
    ...
    async search(query: string): Promise<WorkItem[]> {
      return orgBrainData.workItems.filter(
        (item) =>
          ...
          item.relatedServiceIds?.some((serviceId) => includes(serviceId, query)),
      );
    },
    async getRelated(id: WorkItemId): Promise<WorkItem[]> {
      const item = orgBrainData.workItems.find((candidate) => candidate.id === id);
      if (!item) return [];
      ...
      return orgBrainData.workItems.filter(
        (candidate) => relatedIds.has(candidate.id) || candidate.parentId === item.id,
      );
    },
  ...
  repositories: {
    ...
    async getById(id: RepositoryId): Promise<Repository | null> {
      return orgBrainData.repositories.find((repository) => repository.id === id) ?? null;
    },
    async getCommit(repositoryId: RepositoryId, sha: CommitSha): Promise<Commit | null> {
      return orgBrainData.commits.find(
        (commit) => commit.repositoryId === repositoryId && commit.sha === sha,
      ) ?? null;
    },
    async getSourceSnapshot(
      ...
      return orgBrainData.sourceSnapshots.find(
        (snapshot) =>
  ...
  deployments: {
    async getById(id: DeploymentId): Promise<Deployment | null> {
      return orgBrainData.deployments.find((deployment) => deployment.id === id) ?? null;
    },
    ...
    async getAroundTime(serviceId: ServiceId, timestamp: ISODateString): Promise<Deployment[]> {
      const center = new Date(timestamp).getTime();
      const windowMs = 24 * 60 * 60 * 1000;
      return orgBrainData.deployments.filter(
        (deployment) =>
          deployment.serviceId === serviceId &&
          Math.abs(new Date(deployment.deployedAt).getTime() - center) <= windowMs,
      );
    },
  },
  incidents: {
    ...
    async getById(id: IncidentId): Promise<Incident | null> {
      return orgBrainData.incidents.find((incident) => incident.id === id) ?? null;
    },
  },
  observability: {
    ...
    async getMetrics(serviceId: ServiceId): Promise<MetricComparison[]> {
      return orgBrainData.metrics.filter((metric) => metric.serviceId === serviceId);
    },
  },
  architecture: {
    ...
    async getById(id: ArchitectureDecisionId): Promise<ArchitectureDecision | null> {
      return orgBrainData.architectureDecisions.find((decision) => decision.id === id) ?? null;
    },
